...
contract Liquid {
    ...
    function addMessage(
        ...
        _messagesByAddress[_cidByUsername[username].userAddress].push(
            UserMessage(msg.sender, message, amount, block.timestamp)
        );
    }

    function getAllMessages(
        address userAddress
    ) public view returns (UserMessage[] memory) {
